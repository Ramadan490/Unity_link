...
export default function ProfileScreen() {
  ...
  const { user, users, updateUserRole, isSuperAdmin, isBoardMember, isCommunityMember } =
    useRole();
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      {isSuperAdmin && (
        <Text style={[styles.roleNote, { color: theme.secondaryText }]}>
          {ROLE_LABELS.super_admin}
        </Text>
      )}
      {isBoardMember && (
        <Text style={[styles.roleNote, { color: theme.secondaryText }]}>
          {ROLE_LABELS.board_member}
        </Text>
      )}
      {isCommunityMember && (
        <Text style={[styles.roleNote, { color: theme.secondaryText }]}>
          {ROLE_LABELS.community_member}
        </Text>
      )}
...
const styles = StyleSheet.create({
  ...
  roleNote: { fontSize: 14, fontStyle: 'italic', marginBottom: 10 },
